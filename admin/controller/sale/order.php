...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		/* Tracking Module */
		$tracking_info = $this->model_sale_tracking->getTracking($order_id);
		$submit_payment_info = $this->model_sale_tracking->getSubmitPayment($order_id);
		/* Tracking Module */
		...
		if ($order_info) {
			...
			/** Tracking Module **/
			$this->data['tab_tracking'] = $this->language->get('tab_tracking');
			$this->data['text_tracking_no'] = $this->language->get('text_tracking_no');
			$this->data['text_tracking_url'] = $this->language->get('text_tracking_url');
			$this->data['text_parcel'] = $this->language->get('text_parcel');
			$this->data['text_parcel_description'] = $this->language->get('text_parcel_description');
			$this->data['text_parcel_shipped'] = $this->language->get('text_parcel_shipped');
			$this->data['text_parcel_deliveried'] = $this->language->get('text_parcel_deliveried');
			$this->data['text_tracking_notify'] = $this->language->get('text_tracking_notify');
			$this->data['button_update_tracking'] = $this->language->get('button_update_tracking');
			
			$this->data['tab_submit_transfer'] = $this->language->get('tab_submit_transfer');
			$this->data['text_bank'] = $this->language->get('text_bank');
			$this->data['text_payment_date'] = $this->language->get('text_payment_date');
			$this->data['text_total'] = $this->language->get('text_total');
			$this->data['text_slip_no'] = $this->language->get('text_slip_no');
			...
			$this->data['button_update_transfer'] = $this->language->get('button_update_transfer');
			...
			/* Tracking Module */
			if ($tracking_info) {
				$this->data['tracking_no'] = $tracking_info['tracking_no'];
				$this->data['parcel_type'] = $tracking_info['parcel_type'];
				$this->data['url'] = $tracking_info['url'];
				$this->data['parcel_description'] = $tracking_info['description'];
				$this->data['tracking_notify'] = $tracking_info['status'];
				$this->data['parcel_shipped_date'] = (($tracking_info['shipped_date'])!='0000-00-00 00:00:00')? date('Y/m/d', strtotime($tracking_info['shipped_date'])) : '';
				$this->data['parcel_deliveried_date'] = (($tracking_info['deliveried_date'])!='0000-00-00 00:00:00')? date('Y/m/d', strtotime($tracking_info['deliveried_date'])) : '';
			} else {
				$this->data['tracking_no'] = '';
				$this->data['parcel_type'] = '';
				$this->data['url'] = '';
				$this->data['tracking_notify'] = '0';
				$this->data['parcel_description'] = '';
				$this->data['parcel_shipped_date'] = '';
				$this->data['parcel_deliveried_date'] = '';
			}
			
			if ($submit_payment_info) {
				$this->data['submit_payment'] = true;
				
				$this->data['transfer_order_id'] = $submit_payment_info['order_id'];
				$this->data['transfer_order_date'] = (($submit_payment_info['order_date'])!='0000-00-00 00:00:00')? date('Y-m-d', strtotime($submit_payment_info['order_date'])) : '';
				$this->data['transfer_name'] = $submit_payment_info['name'];
				$this->data['transfer_email'] = $submit_payment_info['email'];
				$this->data['transfer_telephone'] = $submit_payment_info['telephone'];
				$this->data['transfer_bank'] = $submit_payment_info['bank'];
				$this->data['transfer_payment_date'] = (($submit_payment_info['payment_date'])!='0000-00-00 00:00:00')? date('Y-m-d', strtotime($submit_payment_info['payment_date'])) : '';
				$this->data['transfer_total'] = $this->currency->format($submit_payment_info['total'], $this->config->get('config_currency'));//$submit_payment_info['total'];
				$this->data['transfer_slip_no'] = $submit_payment_info['slip_no'];
				$this->data['transfer_payment_method'] = $submit_payment_info['payment_method'];
				$this->data['transfer_remark'] = $submit_payment_info['remark'];
			} else {
				$this->data['submit_payment'] = NULL;
				
				$this->data['transfer_order_id'] = '';
				$this->data['transfer_order_date'] = '';
				$this->data['transfer_name'] = '';
				$this->data['transfer_email'] = '';
				$this->data['transfer_telephone'] = '';
				$this->data['transfer_bank'] = '';
				$this->data['transfer_payment_date'] = '';
				$this->data['transfer_total'] = '';
				$this->data['transfer_slip_no'] = '';
				$this->data['transfer_payment_method'] = $this->language->get('text_deposit');
	...
	public function updatetrackinginfo() {
		$this->language->load('sale/order');
		$this->language->load('sale/tracking');
		...
		$this->model_sale_tracking->updateTracking($order_id, $this->request->post);
		
		$order_query = $this->model_sale_order->getOrder($order_id);
		
		$name = $order_query['firstname'] . " " . $order_query['lastname'];
		$subject = sprintf($this->language->get('text_email_subject'), html_entity_decode($order_query['store_name'], ENT_QUOTES, 'UTF-8'), $order_id);
		
		//$url = $this->config->get('config_url') . 'index.php?route=tracking/board';
		$url = "<a href='" . HTTP_CATALOG . "index.php?route=tracking/board'>" . HTTP_CATALOG . "index.php?route=tracking/board</a>";
		
		$text = $this->language->get('text_email_customer') . "<br />";
		$text .= sprintf($this->language->get('text_email_information'), $name) . "<br /><br />";
		$text .= sprintf($this->language->get('text_email_order_no'), date($this->language->get('date_format_short'), strtotime($order_query['date_added']))) . "<br />";
		$text .= sprintf($this->language->get('text_email_order_date'), $name) . "<br /><br />";
		$text .= sprintf($this->language->get('text_email_tracking_no'), $this->request->post['tracking_no']) . "<br />";
		$text .= sprintf($this->language->get('text_email_parcel'), $this->request->post['parcel_type']) . "<br />";
		$text .= sprintf($this->language->get('text_email_parcel_description'), $this->request->post['parcel_description']) . "<br /><br />";
		$text .= sprintf($this->language->get('text_email_shipping_date'), $this->request->post['shipped_date']) . "<br />";
		$text .= sprintf($this->language->get('text_email_deliveried_date'), $this->request->post['deliveried_date']) . "<br /><br />";
		$text .= $this->language->get('text_email_link') . "<br />";
		$text .= "\n\n";
		$text .= $url . "<br /><br />";
		$text .= $this->language->get('text_email_thank') . "<br /><br />";
		
		$html = "<br /><img src='" . HTTP_CATALOG . "image/". $this->config->get('config_logo') ."' alt='' style='border:none;' ><br /><br />$text";
		...
		$mail->setTo($order_query['email']);
		$mail->setFrom($this->config->get('config_email'));
		$mail->setSender($order_query['store_name']);
	...
	public function updatetransferpayment() {
		...
		$this->model_sale_tracking->updateTransferPayment($order_id, $this->request->post);
